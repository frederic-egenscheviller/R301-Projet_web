...
class Admin{
    ...
    public static function create(array $A_postParams):array{
        $B_receive = Model::create($A_postParams, "admin");
        if($B_receive){
    ...
    public static function delete(int $I_id):array{
        $B_receive = Model::deleteById($I_id, "admin");
        if($B_receive){
    ...
    public static function update(array $A_postParams):array{
        $B_receive = Model::update($A_postParams, "admin");
        if($B_receive){
    ...
    public static function selectById(int $I_id):array{
        $A_receive = Model::selectById($I_id, "admin");
        return $A_receive[0];
    }
